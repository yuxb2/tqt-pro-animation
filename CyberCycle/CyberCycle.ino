 ...
 *               white where the channels agree and split RGB where they do
 *               not - OFF THE LIST for now, put 2 back in viewOrder to
...
#define WR_TEXT    "DON'T TRUST YOUR EYES\x07"   // \x07 is the diamond separator
#define WR_REPEAT  2
...
static void animColorText(float t) {
  ...
  for (int n = 0; n < len; n++) {
    ...
    for (;;) {
      ...
      if (have) {
        line(lx, ly, x, y, col);
        line(lx + 1, ly, x + 1, y, col);   // bold, as on the ring
      }
      lx = x; ly = y; have = true;
    }
  }
